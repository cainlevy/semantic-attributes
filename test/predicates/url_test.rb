require File.dirname(__FILE__) + '/../test_helper'

class UrlPredicateTest < Test::Unit::TestCase
  def setup
    @predicate = Predicates::Url.new
  end

  def test_ip_addresses
    assert_equal true, @predicate.allow_ip_address, 'default allow_ip_address is true'
    assert @predicate.validate('http://127.0.0.1/', nil), 'allow ip address with scheme'
    assert @predicate.validate('192.168.0.10', nil), 'different ip address without scheme'
    assert @predicate.validate('http://www.example.com', nil), 'basic url still works'

    @predicate.allow_ip_address = false
    assert !@predicate.validate('http://127.0.0.1/', nil), 'allow ip address with scheme'
    assert !@predicate.validate('192.168.0.10', nil), 'different ip address without scheme'
    assert @predicate.validate('http://www.example.com', nil), 'basic url still works'
  end

  def test_schemes
    assert_equal ['http', 'https'], @predicate.schemes, 'default allowed schemes'

    assert @predicate.validate('http://example.com/', nil)
    assert !@predicate.validate('ftp://example.com/', nil)
    assert @predicate.validate('www.example.com', nil)

    @predicate.schemes = ['ftp']

    assert !@predicate.validate('http://example.com/', nil)
    assert @predicate.validate('ftp://example.com/', nil)
    assert !@predicate.validate('www.example.com', nil)
  end

  def test_domains
    assert_equal nil, @predicate.domains, 'default allows any domain'

    assert @predicate.validate('example.com', nil)
    assert @predicate.validate('example.co.uk', nil)
    assert @predicate.validate('example.xyz', nil)
    assert @predicate.validate('example', nil)
    assert @predicate.validate('127.0.0.1', nil)

    @predicate.domains = ['com', 'net', 'org']

    assert @predicate.validate('example.com', nil)
    assert !@predicate.validate('example.co.uk', nil)
    assert !@predicate.validate('example.xyz', nil)
    assert !@predicate.validate('example', nil)
    assert !@predicate.validate('127.0.0.1', nil)
  end

  def test_ports
    assert_equal nil, @predicate.ports, 'default allows any port'

    assert @predicate.validate('example.com', nil)
    assert @predicate.validate('example.com:80', nil)
    assert @predicate.validate('example.com:443', nil)

    @predicate.ports = [nil, 80]

    assert @predicate.validate('example.com', nil)
    assert @predicate.validate('example.com:80', nil)
    assert !@predicate.validate('example.com:443', nil)
  end

  def test_bad_url
    assert !@predicate.validate('http:\\\\example.com\\', nil)
  end

  def test_implied_scheme
    assert 'http', @predicate.implied_scheme

    assert @predicate.validate('example.com', nil)
    assert @predicate.validate('example.com:80', nil)

    @predicate.implied_scheme = nil
    assert !@predicate.validate('example.com', nil)
  end
end